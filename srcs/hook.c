...
int key_hook2(int keycode, t_fractol *data)
{
    if (keycode == 50) // Touche '2' pour modifier la couleur en 2050
        ...
        data->color = 265;
    else if (keycode == 52) // Touche '4' pour activer/désactiver le mode "julia_mouse"
        data->julia_mouse = !data->julia_mouse;
    else if (keycode == 53) // Touche '5' pour activer/désactiver l'affichage du texte
...
int key_hook(int keycode, t_fractol *data)
{
    if (keycode == 27) // Touche 'Escape' pour quitter le programme
        exit(1);
    else if (keycode == 43) // Touche '+' pour augmenter la valeur de data->it_max de 50
        data->it_max += 50;
    else if (keycode == 110) // Touche '-' pour diminuer la valeur de data->it_max de 50
        data->it_max -= 50;
        ...
        data->x1 += 30 / data->zoom;
    else if (keycode == 40) // Touche 'fleceh bas' pour déplacer data->y1 vers le bas
